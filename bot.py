...
import base64
BOT_TOKEN = base64.b64decode("ODkzMjA0MTczMjpBQUUzcXEtanYzdDlGcFduZ0NaSFRBbi1KZ3pibWNiX25uMA==").decode()
if not BOT_TOKEN:
...
def sf(v):
    """Safe float - handles strings, #N/A, None, etc."""
...
async def handle_document(update: Update, context):
    ...
    try:
        ...
        # Find the right sheet with warehouse_id
        ws = None
        headers = None
        wh_col_idx = None
...
async def warehouse_callback(update: Update, context):
    ...
    if user_id not in user_data:
        await query.edit_message_text("❌ Pehle Excel file bhejo!")
        return
    ...
    try:
        ...
        if a is None:
            await query.edit_message_text("❌ Koi data nahi mila!")
            return
...
async def report_cmd(update: Update, context):
    ...
    if user_id not in user_data or not user_data[user_id].get('analysis'):
        await update.message.reply_text("❌ Pehle Excel file bhejo aur analyze karo!")
        return
    
    await update.message.reply_text("📥 Excel report generate ho raha hai...")
    logger.info(f"Report requested by user {user_id}")
    logger.info(f"user_data keys: {list(user_data.keys())}")
    if user_id in user_data:
        logger.info(f"user_data[{user_id}] keys: {list(user_data[user_id].keys())}")
